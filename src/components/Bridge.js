...
function Bridge({ ids, setBridgePopup }) {
        ...
        <div className='fixed top-0 right-0 z-30 h-screen w-screen flex items-center justify-center bg-gray-900 bg-opacity-50 select-none'>
            <div className="flex items-center justify-center text-gray-500 md:w-8/12 lg:w-6/12 xl:w-4/12">
                <div className="rounded-xl bg-white shadow-xl w-full px-16 py-5">
                    <h3 className="font-extrabold text-4xl text-primary-800 text-center mt-4 mb-10">Bridge NFTs</h3>
                    <div class="container mx-auto">
                        ...
                        <div className='flex'>
                            <button className={`${onSummit ? "border-primary-500 cursor-default" : "hover:border-primary-500"} group h-12 px-6 mt-10 mx-auto border-2 border-gray-200 rounded-full transition duration-300 w-1/3 disabled:cursor-default disabled:pointer-events-none`} onClick={() => summit()} disabled={onSummit === onSuccess}>
                                <div className="relative flex items-center space-x-4 justify-center">
                                    <span className="block font-semibold tracking-wide">
                                        {
                                            !onSummit && !onSuccess ?
                                                <button className='text-gray-700 text-sm transition duration-300 group-hover:text-primary-500 sm:text-base disabled:text-gray-700'
                    ...
                    <div className="py-10 space-y-2 text-gray-600 text-center sm:-mb-8">
                        <p className="text-xs">NFTs will be {' '}
                            <span className='font-bold'>
                                moved to
                            </span>
                            {' '}
                            another blockchain as you chose.
                        </p>
                        <p className="text-xs">NFTs that mistakenly have same nature as your purpose will not be affected.</p>
                    </div>
                </div>
            </div >
            <div className='h-screen w-screen absolute -z-10' onClick={() => !onSummit && setBridgePopup(false)}></div>
        </div >
    )
}
